import torch
from transformers import AutoTokenizer, AutoModel, Wav2Vec2FeatureExtractor, AutoProcessor
...
class AstralQuantizer(torch.nn.Module):
    def __init__(
        ...
        if ssl_model_name == "waveletdeboshir/gigaam-ctc":
            self.ssl_feature_extractor = AutoProcessor.from_pretrained(ssl_model_name, trust_remote_code=True)
        else:
            self.ssl_feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(ssl_model_name, trust_remote_code=True)

        if skip_ssl:  # in case the same SSL model has been loaded somewhere else
            self.ssl_model = None
        else:
            self.ssl_model = AutoModel.from_pretrained(ssl_model_name, trust_remote_code=True).eval()
            # Check which model structure is present
            ...
            elif hasattr(self.ssl_model, 'model') and hasattr(self.ssl_model.model, 'encoder'):
                self.ssl_model.model.encoder.layers = self.ssl_model.model.encoder.layers[:ssl_output_layer]
                self.ssl_model.model.encoder.layer_norm = torch.nn.Identity()
            else:
                raise AttributeError(f"Model {ssl_model_name} has an unsupported structure. Cannot find encoder attribute.")
    ...
    def forward(self, waves_16k, wave_16k_lens, ssl_model=None):
        ...
        if self.ssl_model_name == "waveletdeboshir/gigaam-ctc":
            last_hidden_states = ssl_fn(**alt_inputs).logits
            feature_lens = torch.tensor(last_hidden_states.size(-2))
